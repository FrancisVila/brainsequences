/**
 * Script to apply migration 0005_add_citations.sql
 * 
 * This script will:
...
console.log('Users columns:', usersInfo.map(col => col.name).join(', '));

const hasShowCitations = usersInfo.some(col => col.name === 'show_citations');
if (hasShowCitations) {
...
const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='citations'").all();
if (tables.length > 0) {
  ...
  console.log('✗ citations table MISSING');
}
